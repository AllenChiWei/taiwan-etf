...
`scripts/fetch_yields.py` → `app/public/data/yields.json`（進版控）。
    ...
    配息史滿一年    殖利率 = 近 365 天的公告配息合計 ÷ 當日收盤價
    配息史未滿一年  殖利率 = 最近一次配息 × 每年次數 ÷ 當日收盤價   （est=true）
...
**不滿一年的要年化，而且要標記。** 原本一律用實際近 12 個月，對今年才上市的標的會
低到失真 —— 00400A（月配、7 月才第一次配）顯示 2.32%，實際水準是 9.30%；00406A 只配
過兩次，顯示 2.70% 而不是 16.83%。全站 36 檔是這種情況，幾乎全是今年上市的主動式
ETF，整片看起來像壞掉（使用者就是這樣回報的）。滿一年的**仍然用實際金額**，不會被
某一次加發灌水（00888：實際 10.54%，但最後一次 × 4 會是 19.39%）。

每年配息次數優先從**實際除息日的間隔**推（中位數 30 天 → 12 次），只配過一次推不出
來的才退回 `etfs.json` 既有的頻率標籤。`est` 一路傳到 `etfs.json` 的 `yest`，畫面上
數字旁標一個 `*`（手機沒有 hover，所以清單上另外寫一行說明）。
`verify_data.py` 擋「標了 yest 卻沒有數字」與「一半以上都是推估」。

**近一年沒配過息的輸出 null 而不是 0%**（「沒配」與「配了 0」不是同一件事，這與台股頁
對 N/A 的既有處理一致）。

舊版單頁 `taiwan_etf_list.html` 只顯示數字、不標 `*` —— 它是維護模式，而且那一欄的
排序是直接解析儲存格文字。
