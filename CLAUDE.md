...
- 當月用證交所 OpenAPI 的 `indicesReport/MI_5MINS_HIST`，**一個請求給整個月**。
- 更早的月份只能一天一個請求（`MI_INDEX?type=IND`），所以只補缺的，而且一次最多
  20 天 —— 證交所擋太快的請求時回 **307**（不是 429，也沒有 Retry-After），而且擋住
  之後連原本查得到的日期也一起擋。一次跑 50 個就踩到了，隔天再補反而快。
