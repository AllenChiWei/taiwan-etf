...
import {
  projectHolding, buildPortfolio, inferFrequency, payoutsPerYear, SHARES_PER_LOT,
...
test('年配息 = 最近一次 × 一年幾次，不是把過去一年加總', () => {
  ...
  const r = projectHolding(series({ d }), MONTHS, 10);
...
test('配息落在實際發生的月份，不是機械式平均分配', () => {
  ...
  d[7] = 0.5;   // 8月
  d[10] = 0.5;  // 11月
  const r = projectHolding(series({ d }), MONTHS, 1);
...
test('月份加總等於年配息', () => {
  ...
  const r = projectHolding(series({ d }), MONTHS, 3);
...
test('月配 12 個月都有', () => {
  const r = projectHolding(series({ d: new Array(12).fill(0.1) }), MONTHS, 2);
...
test('推估次數多於實際記錄到的月份時，差額補到其他月份讓總和對得起來', () => {
  // 只記錄到 11 個月有配息，但會被判成月配（一年 12 次）
  const d = new Array(12).fill(0.1);
  d[5] = 0;
  const r = projectHolding(series({ d }), MONTHS, 1);
  assert.equal(r.payouts, 11);
  assert.equal(r.freq, '月配');
  assert.equal(r.perYear, 12);
  assert.equal(r.byMonth.filter(v => v > 0).length, 12, '補滿 12 個月');
  const sum = r.byMonth.reduce((a, b) => a + b, 0);
  assert.ok(Math.abs(sum - r.perShare) < 1e-9);
});

test('殖利率用推估的年配息除以最新股價', () => {
  const d = new Array(12).fill(0); d[5] = 2.0;
  const r = projectHolding(series({ d, last: { date: '2025-12-31', close: 25 } }),
    MONTHS, 1);
...
test('資料不滿一年時據實回報月數', () => {
  ...
  assert.deepEqual(r.payoutMonths, [8, 11]);
});

test('投資組合把各檔加總到同一份月曆', () => {
  const dA = new Array(12).fill(0); dA[0] = 1; dA[6] = 1;        // 半年配
  const dB = new Array(12).fill(0.05);                          // 月配
  const a = projectHolding(series({ code: 'A', d: dA }), MONTHS, 1);
  const b = projectHolding(
    series({ code: 'B', d: dB, last: { date: '2025-12-31', close: 10 } }), MONTHS, 2);
...
test('近一年沒配息的標的不會產生假數字', () => {
  const r = projectHolding(series({ d: new Array(12).fill(0) }), MONTHS, 10);
